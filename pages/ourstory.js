...
const OurStoryPage = ({ classes, ...props }) => {
  // console.log(props);
  const schoolCode = 'OCM';

  return (
    <Page>
      <Query query={query} variables={{ schoolCode, path: '/our-story' }}>
        {({ error, loading, data }) => {
          if (error) {
            console.error(error);
            return (
...
export default withStyles(styles)(OurStoryPage);
