...
import BecomePartner from '../ui/becomeapartner';
...
const BecomeAPartnerPage = ({ classes, ...props }) => {
  // console.log(props);
  ...
  return (
    <Page>
      <Query
        query={query}
        variables={{ schoolCode, path: '/become-a-partner' }}
      >
        {({ error, loading, data }) => {
          if (error) {
            console.error(error);
          ...
          return (
            <Grid container className={classes.container}>
              <BecomePartner page={content} {...props} />
            </Grid>
          );
        }}
      </Query>
...
BecomeAPartnerPage.propTypes = {
  classes: PropTypes.object.isRequired
};

export default withStyles(styles)(BecomeAPartnerPage);
